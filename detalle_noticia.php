...
// Obtener datos de la noticia de propuestas_noticias
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Publicar noticia
    if (isset($_POST['publicar']) && isset($_POST['id_noticia'])) {
        $id_publicar = intval($_POST['id_noticia']);
        $titulo_noticia = trim($_POST['titulo_noticia'] ?? '');
        ...
        if ($propuesta) {
            // Actualizar estado en propuestas_noticias
...
<div class="modal-volver" id="modalVolver">
    <div class="modal-volver-contenedor">
        <h2 class="modal-volver-titulo">¿Estás seguro de regresar a las noticias pendientes?</h2>
...
<div class="modal-publicar" id="modalPublicar">
    <div class="modal-publicar-contenedor">
        <h2 class="modal-publicar-titulo">¿Estás seguro de publicar esta noticia?</h2>
        <p class="modal-publicar-texto">Estás a punto de publicar esta noticia. Una vez publicada, estará disponible para todos los lectores.</p>
...
<div id="alertaPublicacion" class="alerta-exito">
    Noticia publicada con éxito, disponible para los lectores
</div>
...
function confirmarVolver() {
    window.location.href = 'revision_noticias.php';
}
...
function confirmarPublicarNoticia() {
    ...
    const tituloNoticia = "<?= addslashes($noticia['titulo']) ?>";
    const autorId = 0;
    
    // Cerrar el modal
    cerrarModalPublicar();
    
    // Enviar solicitud para publicar la noticia
    fetch('', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'publicar=1&id_noticia=' + idNoticia + '&titulo_noticia=' + encodeURIComponent(tituloNoticia) + '&autor_id=' + autorId
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            // Redirigir a revision_noticias.php después de 2 segundos
            setTimeout(() => {
                window.location.href = 'revision_noticias.php';
            }, 2000);
        } else {
...
function confirmarEliminarNoticia() {
    ...
    // Crear formulario oculto para enviar datos a rechazar_noticias.php
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = 'rechazar_noticias.php';
