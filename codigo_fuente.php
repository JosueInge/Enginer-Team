...
function imagenExiste($nombre_imagen) {
  return !empty($nombre_imagen) && file_exists('imagenes/denuncias/' . $nombre_imagen);
}

function obtenerImagenDenuncia($denuncia) {
  $imagenes = [];
  foreach (['imagen', 'imagen2', 'imagen3'] as $campo) {
    if (!empty($denuncia[$campo]) && imagenExiste($denuncia[$campo])) {
...
// --- Consulta de las últimas 8 denuncias aprobadas ---
$sql_denuncias = "
  SELECT pd.*, u.nombre AS autor 
...
<head>
  ...
  <title>Denuncias Ciudadanas - Comunicado Digital</title>
...
<div class="container-fluid">
  <section class="seccion-ultimas-denuncias">
    <h2 class="titulo-ultimas-denuncias">Últimas denuncias</h2>
