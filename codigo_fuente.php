...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { margin:0; font-family: 'Inter', sans-serif; background:#fff; color:#061F3E; }
    header { background:#061F3E; color:#fff; padding:12px 18px; display:flex; justify-content:space-between; align-items:center; }
    .logo img { height:40px; }
    nav a { color:#fff; text-decoration:none; font-weight:600; }

    .contenedor-principal { max-width:820px; margin:28px auto; padding:18px; }
    h1 { font-family:'Poppins',sans-serif; color:#1661AC; text-align:center; margin-bottom:18px; font-size:26px; }

    .campo { margin-bottom:18px; position:relative; }
    .campo label { display:block; margin-bottom:6px; font-weight:600; color:#403F48; }
    .campo input[type="text"], .campo input[type="date"], .campo select, .campo textarea {
      width:100%; padding:10px 12px; border:1px solid #B1B1B1; border-radius:10px; font-size:15px; outline:none;
      transition: box-shadow .12s, border-color .12s;
      padding-right:60px;
    }
    .campo textarea { min-height:140px; resize:vertical; }

    .campo input:focus, .campo textarea:focus, .campo select:focus { border-color:#2D8EFF; box-shadow: 0 4px 14px rgba(45,142,255,0.08); }

    .contador { position:absolute; right:12px; bottom:10px; font-size:12px; color:#8A8A8A; display:none; pointer-events:none; }
    .error { color:#E33629; font-size:13px; margin-top:6px; min-height:18px; }
...
document.getElementById('volverNoticias').addEventListener('click', (e) => {
  e.preventDefault();
  abrirModal('modalPublicar'); // si quieres otro modal para volver, puedes crear otro similar
});
